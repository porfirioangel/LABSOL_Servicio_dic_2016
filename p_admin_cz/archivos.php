...
$archivo = "archivos.php";// El nombre del archivo donde tenemos este codigo
$hasta = 10;
# <-| Opciones
$pagina = $_GET["pagina"];
if (!$pagina) {
...
$resultados = mysql_query('SELECT ponentes.nombre, ponentes.apellidos,  archivos.descripcion, archivos.ruta, archivos.nom, archivos.fecha
FROM ponentes
INNER JOIN archivos ON ponentes.id_ponentes = archivos.id_ponentes where '.$opcion.' like "'. $criterio .'%"  order by fecha asc');  
...
$resultados = mysql_query("SELECT ponentes.nombre, ponentes.apellidos,  archivos.descripcion, archivos.ruta, archivos.nom, archivos.fecha
FROM ponentes
INNER JOIN archivos ON ponentes.id_ponentes = archivos.id_ponentes order by fecha asc");
}
...
$resultados = mysql_query('SELECT ponentes.nombre, ponentes.apellidos,  archivos.descripcion, archivos.ruta, archivos.nom, archivos.fecha
FROM ponentes
INNER JOIN archivos ON ponentes.id_ponentes = archivos.id_ponentes where '.$opcion.' like "'. $criterio .'%"'. "order by fecha asc limit " . $desde . "," . $numpag);  }
else{
$resultados = mysql_query('SELECT ponentes.nombre, ponentes.apellidos,  archivos.descripcion, archivos.ruta, archivos.nom, archivos.fecha
FROM ponentes
INNER JOIN archivos ON ponentes.id_ponentes = archivos.id_ponentes order by fecha asc  '. " limit " . $desde . "," . $numpag);   //LIMIT $desde, $numpag);   //LIMIT $desde, $numpag");
}
...
<table border="0" cellpadding="8" cellspacing="2" bordercolor="#ccc" bgcolor="#CCCCCC" >
  <tr bgcolor="#CCCCCC"  class="c">
   <td height="30"><div align="center"><strong>Nombre</strong></div></td>
		<td height="30"><div align="center"><strong>Archivo</strong></div></td>
		<td height="30"><div align="center"><strong>Descripcion</strong></div></td>
		<td height="30"><div align="center"><strong>Fecha</strong></div></td>
...
$ac="baja";}else{$ac="activo";}
  ...
  printf("<tr class='dd' ><td>%s</td><td>%s<a class='dd' title='Ver Archivo' href=../reg_ponentes/".$row["ruta"]." target='_blank'>".$row["nom"]."</a></td><td>%s</td><td>%s</td></tr>",$row["nombre"]." ".$row["apellidos"], "",$row["descripcion"],$row["fecha"]);
