...
if ($_POST['action'] == "Guardar") {
		...
		if ($_GET['dp']!=""){
			...
			$d->modificar($_POST['txtnom'],$_POST['txtcan'], $_POST['txtfecha'],$_POST['txthora'],$_POST['txtcor'],$_POST['txtlugar'],$_GET['dp']);
			echo '<BODY onLoad="javascript:index()">';  
			}
		...
		$nn=$d->abrir($_POST['txtnom']);
		
		if ($nn==0){
			
		
$d->guardar($_POST['txtnom'],$_POST['txtcan'], $_POST['txtfecha'],$_POST['txthora'],$_POST['txtcor'],$_POST['txtlugar']) ;
...
function validacion(formulario) {
	...
	var er_numbers = /^([0-9]|.)+$/            
		var er_email = /^(.+\@.+\..+)$/
			...
			if(frmact.txtnom.value=="") {    
    		    alert('Nombre  no válido.')   
      		  return false   
    		}  
			
				if(frmact.txtcan.value=="") {    
    		    alert('Cantidad no válido.')   
      		  return false   
    		}  
			
				if(frmact.txtfecha.value=="") {    
    		    alert('Fecha no válido.')   
      		  return false   
    		}  
			
			if(frmact.txthora.value=="") {    
    		    alert('Hora no válido.')   
      		  return false   
    		}  
			
			if(frmact.txtcor.value=="") {    
    		    alert('Coordinador no válido.')   
      		  return false   
    		}  
			
			if(frmact.txtlugar.value=="") {    
...
<div class="post" id="post-6">
 ...
 echo "Bienvenido:"." ".$_SESSION['p_noma'];
   ...
   <table width="423" height="255" border="0">
  <tr>
    <td width="417"><form id="frmact" name="frmact" method="post" action="" onsubmit="return validacion(this)">
      <p>&nbsp;</p>
      
      <table width="399" border="0" align="center">
        <tr>
          <td colspan="2" class="c">Talleres</td>
          </tr>
        
        
        <tr>
          <td height="62" class="d"><div align="center">Nombre del Taller:</div></td>
          <td width="232" height="62" class="d"><label for="txtnom_can"></label>
            <input name="txtnom" type="text" id="txtnom"  <?php echo 'value="'.$dat['nombre'].'"'  ?> size="30" /></td>
          </tr>
           <tr>
          <td height="62" class="d"><div align="center">Cantidad:</div></td>
          <td width="232" height="62" class="d"><label for="txtnom_can"></label>
            <input name="txtcan" type="text" id="txtcan"  <?php echo 'value="'.$dat['cantidad'].'"'  ?> size="30" /></td>
          </tr>
           <tr>
          <td height="62" class="d"><div align="center">Fecha:(año-mes-dia)</div></td>
          <td width="232" height="62" class="d"><p>
            <label for="txtnom_can"></label>
            <input name="txtfecha" type="text" id="txtfecha"  <?php echo 'value="'.$dat['fecha'].'"'  ?> size="30" />
          </p>
           </td>
          </tr>
           <tr>
          <td height="62" class="d"><div align="center">Hora:</div></td>
          <td width="232" height="62" class="d"><label for="txttel"></label>
            <input name="txthora" type="text" id="txthora"  <?php echo 'value="'.$dat['hora'].'"'  ?> size="30" /></td>
          </tr>
           <tr>
          <td height="62" class="d"><div align="center">Coordinador:</div></td>
          <td width="232" height="62" class="d"><label for="txttel"></label>
            <input name="txtcor" type="text" id="txtcor"  <?php echo 'value="'.$dat['coordinador'].'"'  ?> size="30" /></td>
          </tr>
           <tr>
          <td height="62" class="d"><div align="center">Lugar:</div></td>
          <td width="232" height="62" class="d"><label for="txtmail"></label>
            <input name="txtlugar" type="text" id="txtlugar"  <?php echo 'value="'.$dat['lugar'].'"'  ?> size="30" /></td>
          </tr>
        <tr>
          <td colspan="2"><div align="center">
            ...
            <input type="reset" name="but" id="but" value="Nuevo"  />
...
# Opciones |->
$numpag = 20;// Resultados a mostrar por pagina
$adicionales = 3;//Resultados a mostrar mayores y menores que la pagina actual
$archivo = "taller.php";// El nombre del archivo donde tenemos este codigo
$hasta = 20;
# <-| Opciones
$pagina = $_GET["pagina"];
if (!$pagina) {
...
$resultados = mysql_query('SELECT * from taller where taller.nombre like "%'. $criterio .'%"  order by taller.fecha asc');  
...
$resultados = mysql_query("SELECT * from taller order by taller.fecha asc");
}
$total_registros = mysql_num_rows($resultados);
$total_paginas = ceil($total_registros / $numpag); 			 
if ($criterio!=""){
$resultados = mysql_query('SELECT * from taller where taller.nombre like "%'. $criterio .'%" order by taller.fecha asc '." limit " . $desde . "," . $numpag);  }
else{
$resultados = mysql_query('SELECT * from taller order by taller.fecha asc '." limit " . $desde . "," . $numpag);   //LIMIT $desde, $numpag");
}
...
printf("<tr class='dd'><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td>
<td align='center' ><a title='Editar' href=taller.php?dp=%d><img src='images/edit.jpg' width='16' height='16'></a></td>
<td align='center' ><a title='Eliminar' href=javascript:confirmDelete('taller.php?dl=%d')><img src='images/del.jpg'  width='16' height='16'></a></td>

</tr>",$row["nombre"],(25-$row["cantidad"]),$row["fecha"],$row["hora"],$row["coordinador"],$row["lugar"],$row["id_taller"],$row["id_taller"]);
}
