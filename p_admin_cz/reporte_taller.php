...
$this->Text(70,15,'	LABSOL & Campus Party, Zacatecas 2015');
if ($tl=="Todos"){
...
FROM taller INNER JOIN (asistentes INNER JOIN expediente ON asistentes.matricula = expediente.matricula) ON taller.id_taller = expediente.id_taller where asistentes.activo=1" ; }
...
FROM taller INNER JOIN (asistentes INNER JOIN expediente ON asistentes.matricula = expediente.matricula) ON taller.id_taller = expediente.id_taller where taller.id_taller=".$tl." and asistentes.activo=1" ;
		...
		if ($tl=="Todos"){
$this->Text(70,23,'Reporte de Registrados a Todos Talleres.     Registrados: '.$num_r);
		}elseif($tl==""){}else{
...
$this->Cell(70,5,'Foro',1,0,'C');
$this->Cell(17,5,'Fecha Insc.',1,0,'C');
...
FROM taller INNER JOIN (asistentes INNER JOIN expediente ON asistentes.matricula = expediente.matricula) ON taller.id_taller = expediente.id_taller where asistentes.activo=1 order by taller.nombre " ; }elseif($tl==""){}else{
...
while($row = mysql_fetch_array($result1a))
	...
	$pdf->Row(array($row["matricula"],utf8_decode(ucwords(strtolower($row["apellidos"]))).' '.utf8_decode(ucwords(strtolower($row["nombre"]))),$row["email"],utf8_decode(ucwords(strtolower($row["institucion"]))),$row["telefono"],utf8_decode(ucwords(strtolower($row["taller"]))),substr($row['fecha_ins'],0,10)));
