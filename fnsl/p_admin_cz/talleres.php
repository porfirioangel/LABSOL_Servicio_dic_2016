...
window.location="talleres.php?tal="+a; 
...
if ($_GET['b']!="" and $_GET['t']!="" or $_GET['a']!="" and $_GET['t']!="" ){
		
//$r=$d->abrir_mat($m);
$val=$d->abrir_tipo($mt,$_GET['t']);


if ($val>=1){
	//echo "update asistencia_taller set asistio=".$ins." where matricula=".$mt." and id_taller=".$_GET['t'];
	//exit();
	mysql_query("update asistencia_taller set asistio=".$ins." where matricula=".$mt." and id_taller=".$_GET['t'] ,$link);
}
...
if ($val==0){
	...
	//exit();
	mysql_query("insert into asistencia_taller values ('',$mt,$ins,'$fecha',".$_GET['t'].",'')" ,$link);
}
...
$archivo = "talleres.php";// El nombre del archivo donde tenemos este codigo
$hasta = 10;
# <-| Opciones
$pagina = $_GET["pagina"];
if (!$pagina) {
...
$resultados = mysql_query('select * from asistentes where '.$opcion.' like "'. $criterio .'%"  order by matricula asc');  
...
$resultados = mysql_query("select * from asistentes inner join expediente on asistentes.matricula=expediente.matricula  where expediente.id_taller=".$tt." order by asistentes.apellidos asc");
}
$total_registros = mysql_num_rows($resultados);
$total_paginas = ceil($total_registros / $numpag); 			 
if ($criterio!=""){
$resultados = mysql_query('select * from asistentes where '.$opcion.' like "'. $criterio .'%"'. "order by matricula asc limit " . $desde . "," . $numpag);  }
else{
$resultados = mysql_query('select * from asistentes asistentes inner join expediente on asistentes.matricula=expediente.matricula  where expediente.id_taller='.$tt.' order by asistentes.apellidos asc  '. " limit " . $desde . "," . $numpag);   //LIMIT $desde, $numpag);   //LIMIT $desde, $numpag");
}
...
$desp=@mysql_query("select * from asistencia_taller where matricula=".$row['matricula']." and id_taller=".$tt);
...
$ac=" ";}
  ...
  <td>%s</td>
  <td>%s</td>
  <td><a href=\"talleres.php?b=%s\">%s</a></td>
  <td><a href=\"talleres.php?a=%s\">%s</a></td>
